...
async def daily_task(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for user in users["users"]:
            ...
            message = await context.bot.send_message(chat_id=chat_id, text=f"Today is {today}. Loading latest update...")
            ...
            for city in cities_response["list"]:
                ...
                if isinstance(city["results"], list):
                    for house in city["results"]:
                        # Format message
                        message_text = "*{}*\n{}\nPrice: {}\nAvailable Date: {}".format(city["city_name"], house["name"], house["price"], house["available_date"])
                    ...
                    await context.bot.send_message(chat_id=chat_id, text="*{}*\nNo house found".format(city["city_name"]), parse_mode='Markdown')
            
            # Edit loading message to say finished
            await context.bot.edit_message_text(chat_id=chat_id, message_id=message.message_id, text=f"Today is {today}. Here is the latest update:")
...
async def set_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "error" in user and user["error"] == "User not found":
...
async def unset_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        requests.delete(f"{base_url}/users/{chat_id}")
        await context.bot.send_message(
            chat_id=chat_id,
...
async def test_task(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=1949747267, text="Test task ran at: " + str(datetime.now()))
